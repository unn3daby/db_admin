...
import columns from './colums/fundsColumns';
...
const onCreate = async () => {
  try {
    ...
    const {
      data: { message },
    } = await axios.post('http://localhost:8001/api/fund', newRow.value);
...
const onUpdate = async (id: number) => {
  try {
    ...
    const {
      ...
      `http://localhost:8001/api/fund/${id}`,
      changeableRow.value,
    );
    $q.notify({ type: 'positive', message: message });
...
const onDelete = async (id: number) => {
  try {
    ...
    const {
      data: { message },
    } = await axios.delete(`http://localhost:8001/api/fund/${id}`);
    $q.notify({ type: 'positive', message: message });
    await getData();
  } catch (error) {
    console.error(error);
...
const getData = async () => {
  try {
    ...
    const {
      data: { data, message },
    } = await axios.get('http://localhost:8001/api/fund');
    rows.value = data;
    return message;
  } catch (error) {
    console.error(error);
