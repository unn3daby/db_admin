...
import columns from './colums/companiesColumns';
...
const onCreate = async () => {
  try {
    ...
    const {
      data: { message },
    } = await axios.post('http://localhost:8001/api/company', newRow.value);
...
const onUpdate = async (id: number) => {
  try {
    ...
    const {
      ...
      `http://localhost:8001/api/company/${id}`,
      changeableRow.value,
    );
    $q.notify({ type: 'positive', message: message });
...
const onDelete = async (id: number) => {
  try {
    ...
    const {
      data: { message },
    } = await axios.delete(`http://localhost:8001/api/company/${id}`);
    $q.notify({ type: 'positive', message: message });
    await getData();
  } catch (error) {
    console.error(error);
...
const getData = async () => {
  try {
    ...
    const {
      data: { data, message },
    } = await axios.get('http://localhost:8001/api/company');
    rows.value = data;
    return message;
  } catch (error) {
    console.error(error);
