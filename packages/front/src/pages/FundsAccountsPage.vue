<template>
  <q-page class="q-pa-md page-hack flex justify-center items-center">
    ...
    >
      <template #top-left>
        <div class="text-h5">Счета Фондов</div>
        <q-input
          style="width: 150px"
          v-model="search"
          class="q-ml-md"
...
import columns from './colums/fundsAccountColumns';
...
const onCreate = async () => {
  try {
    ...
    const {
      data: { message },
    } = await axios.post('http://localhost:8001/api/fundAccount', newRow.value);
...
const onUpdate = async (id: number) => {
  try {
    ...
    const {
      ...
      `http://localhost:8001/api/fundAccount/${id}`,
      changeableRow.value,
    );
    $q.notify({ type: 'positive', message: message });
...
const onDelete = async (id: number) => {
  try {
    ...
    const {
      data: { message },
    } = await axios.delete(`http://localhost:8001/api/fundAccount/${id}`);
    $q.notify({ type: 'positive', message: message });
    await getData();
  } catch (error) {
    console.error(error);
...
const getData = async () => {
  try {
    ...
    const {
      data: { data, message },
    } = await axios.get('http://localhost:8001/api/fundAccount');
    rows.value = data;
    return message;
  } catch (error) {
    console.error(error);
